...
class HVC(nn.Module):
    ...
    def hybrid_loss(self, q, k, coord_q, coord_k, pos_radius=0.5):
        ...
        # sparse loss
        # [bs, 1024, 1024]
        sparse_logit = torch.bmm(q.view(N, C, -1).transpose(1, 2), k.view(N, C, -1))
        sparse_loss = (sparse_logit * pos_mask).sum(-1).sum(-1) / (pos_mask.sum(-1).sum(-1) + 1e-6)

        # dense loss
        # [bs, 2, 32, 32]
        flow_f = F.normalize(self.flow(q, k), dim=1)
        flow_b = F.normalize(self.flow(k, q), dim=1)
        dense_logit = torch.bmm(flow_f.view(N, 2, -1).transpose(1, 2), flow_b.view(N, 2, -1))
        dense_loss = (dense_logit * pos_mask).sum(-1).sum(-1) / (pos_mask.sum(-1).sum(-1) + 1e-6)

        return -2 * (sparse_loss.mean() + dense_loss.mean())
